...
??? tip "💡 Big Idea"
    ...
    The emergence of $r(t)$ from $N$ independent units corresponds with the **binding problem** in consciouslness studies:
    
    - How do discrite neural oscillators (micro) give rise to a unified conscious experience (macro)?
    - The transistion from $r\approx 0$ to $r\approx 1$ represents a **phase transition**, analogous to symmetry breaking in physics.
    - In the conventional Kuramoto model, this transition occurs at a critial coupling $K_c$ where $r(t) < K_c$ corresponds with a disordered system and $r(t) > K_c$ with spontaneously emerging synchronization (ordered system).
